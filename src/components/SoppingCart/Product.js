...
const Product = ({data, addToCart, styles}) => {
  ...
  return (
    <figure className={cardStyle}>
        ...
        <h3><b>{name}</b></h3>
        <h5 >$ {price}</h5>
        <button className= {buttonStyle} onClick ={() => setShowModal(true)}>Ver</button>
        <Modal isVisible={showModal} onClose={() => setShowModal(false)} data = {data} addToCart={addToCart} styles={styles}/>
    </figure>
